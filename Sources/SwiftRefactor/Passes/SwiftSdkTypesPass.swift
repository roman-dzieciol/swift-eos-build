...
public class SwiftSdkTypesPass: SwiftRefactorPass {
    ...
    public override func refactor(module: SwiftModule) throws {
        ...
        module.replaceWithSdkRefs { decl in
            decl.name.hasSuffix("EOS_UI_ReportKeyEventOptions")
        }

        // EOS_UI_ReportKeyEventOptions is unused
        module.replaceWithSdkRefs { decl in
            decl.name.hasSuffix("EOS_UI_PrePresentOptions")
        }
...
//        }
        ...
        module.replaceWithSdkRefs { decl in
            (decl as? SwiftObject)?.name.hasSuffix("EOS_P2P_PacketQueueInfo") == true
        }
